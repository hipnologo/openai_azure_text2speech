...
def get_openai_text(prompt, limit_chars):
    ...
    return response['choices'][0]['text']  # Update this line
...
def main():
    ...
    st.subheader("Exploring OpenAI and Azure API's")
    st.info("This app uses the OpenAI API to generate text and the Azure API to convert the text to speech. The app also allows you to upload a text file (.txt) or enter a URL of a webpage to generate text from. The app also allows you to select a voice from the dropdown menu. The default voice is 'en-US-AriaNeural'.")
    ...
    if prompt_text:
        # Truncate the prompt_text to ensure the total tokens do not exceed the model's limit
